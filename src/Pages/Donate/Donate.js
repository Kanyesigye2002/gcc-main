...
export default function DetailedAccordion() {
    ...
    const sendOTP = (response) => {
        ...
        axios({
            "method": 'POST',
            "url": `${Url}/api/gcc/v1/givesd`,
            data: {link: `${response.data.data.link}?solution=${otp}`}
        }).then(res => {
            console.log(res)
        })
    }
    ...
    return (
        <>
                            ...
                            <Grid item style={{maxWidth: 400}}>
                                <form onSubmit={onSubmit} className={classes.root} noValidate autoComplete="off">
                                    <Grid item container justifyContent="center" style={{border: "1px solid burlywood"}}
                                        ...
                                        <Grid container justifyContent="space-between" style={{padding: 10, paddingTop: 30}}>
            ...
            <Dialog onClose={() => console.log("Coloes")} aria-labelledby="customized-dialog-title" open={open}>
                ...
                <DialogContent dividers>
                    <Typography>
                        Please enter OTP sent to {data.number}
                    </Typography>
                    <TextField label="OTP" type="text" name="otp"
                               variant="filled"
                               onChange={otpChange} fullWidth/>
                </DialogContent>
                <DialogActions>
                    <Button autoFocus onClick={() => sendOTP(res)} color="primary">
                        Send
                    </Button>
                </DialogActions>
            </Dialog>
